...
import static com.valai.school.utils.AppConstants.ROOT;
...
public class ApiClient {
    ...
    public RestClient getClient(String appendedUrl) {
        Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ssZ").setLenient().create();
        ...
        if (retrofit == null) {
            retrofit = new Retrofit.Builder().baseUrl(ROOT_DOWNLOAD + appendedUrl)
                    .client(httpClient)
                    .addConverterFactory(GsonConverterFactory.create(gson))
                    .build();
        }
